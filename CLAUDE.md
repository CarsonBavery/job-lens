...
Core data entities (Postgres): `profiles`, `resumes`, `cover_letters`, `job_postings` (+ `job_sources`), `applications` (links a posting to the resume/cover-letter version used + status), `subscriptions`, `projects`/`education`/`work_experience` (career profile). RLS is on for every user-owned table; `job_postings`/`job_sources` are public-read, service-role-write only.
