...
**Status:** Phase 1 + Phase 2 merged to `main`. Phase 3 (job board ingestion) built on branch `phase-3`, not yet merged: 4 ATS connectors (Greenhouse, Lever, Ashby, Workable), an embedding-based dedup pipeline, a secret-protected cron route, and a job search UI. Match scoring stays deferred to its own follow-up (see `PROGRESS.md`). `lint`/`tsc`/`test`/`build` all pass; e2e is 3/4 green — the 4th (`ai-features.spec.ts`, unchanged Phase 2 functionality) is blocked today by an **exhausted Gemini free-tier daily quota** (20 generateContent requests/day) from this session's own testing volume, not a code defect — see the Gemini rate-limit notes below and in `PROGRESS.md` before assuming this needs code changes.
...
- `lib/gemini/retryOnRateLimit.ts` — wraps a Gemini call with retry-on-429 logic: parses Google's own `retryDelay` from the error body when present, otherwise falls back to a ~66s wait (long enough to clear a per-*minute* quota window — see "Gemini rate limits" below). Every Gemini call site (`generateBlocks.ts`, `lib/ingestion/embed.ts`) goes through this; don't call the SDK directly from new code.
...
- `lib/ingestion/sources/{greenhouse,lever,ashby,workable}.ts` — one connector per ATS, no auth required for any of them. Workable's list endpoint doesn't include descriptions (always `null` there); Greenhouse's `content` field is HTML-*encoded* (`&lt;div&gt;`), not raw HTML — decode entities before stripping tags
...
- `lib/ingestion/run.ts` — orchestrates fetch → normalize → batch-embed → upsert → dedup for each configured company; safe to re-run (upserts on `source_id`+`external_id`)
- `app/api/cron/ingest/route.ts` — triggers `runIngestion()`, gated on `Authorization: Bearer $CRON_SECRET`
- `app/dashboard/jobs/page.tsx` — search/browse UI; user search input is sanitized before going into a PostgREST `.or()` filter string (raw interpolation there is a filter-injection risk, not just an XSS one)
...
**Gemini rate limits — read before assuming an e2e failure is a code bug:** this project's free-tier API key has (at least) two separate quotas that were both hit live during Phase 3 testing, not in theory:
- `embed_content`: ~100 requests/minute. Fixed by batching (`lib/ingestion/embed.ts` chunks at Gemini's 100-texts-per-call cap) and `retryOnRateLimit` waiting out the window on a 429.
- `generate_content` on whatever model `gemini-flash-latest` currently resolves to: **20 requests/day** on the free tier. A per-*day* quota cannot be fixed by retrying within a test run — `retryOnRateLimit` will wait out its backoff and still fail once the day's budget is gone. If `ai-features.spec.ts` (or any generateContent-based flow) starts failing with a `RESOURCE_EXHAUSTED` whose `quotaId` contains `PerDay`, that's the account, not the code — check `ai.dev/rate-limit` for the actual remaining quota before debugging further, and consider a paid tier before relying on this suite running repeatedly in a single day (e.g. in CI).
...
- **No per-user rate limiting exists yet** on Gemini-calling actions (`tailorResume`, `generateCoverLetter`) — a free-tier user can currently generate unlimited AI content at real API cost with no monetization gate. This is flagged as a pre-launch blocker in `PROGRESS.md`, not fixed yet; don't assume it's handled. (Separately, every Gemini call site *does* now retry on a 429 from Google's own side — see `lib/gemini/retryOnRateLimit.ts` — which is about resilience to the API's rate limit, not about limiting our own users.)
