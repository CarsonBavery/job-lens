...
**Status:** Phases 1–6 are merged to `main` (auth/documents, AI tailoring, job board ingestion + match scoring, career profile, billing — Stripe checkout/portal/webhooks on a **test-mode** account + per-user AI rate limiting), plus a job-posting-staleness fix (`job_postings.status`, soft-delete since `applications` references those rows). Phase 7 (Application tracking — save/apply/status pipeline, linking a specific resume/cover-letter version to each application, plus the removal-from-board + in-app notification when a saved job's posting closes) is now in progress on branch `phase-7`. **The roadmap was restructured 2026-07-27**: front-end visual polish (previously a single flagged bullet under Hardening) was promoted to its own **Phase 8**, pushing Pre-launch hardening→9 and Launch→10 — see "Billing (Stripe)" below for Phase 6 setup details, and `PROGRESS.md` for the full backlog and session history. The Gemini API key is on a paid tier as of 2026-07-26 (removed the free tier's 20-requests/day cap). See "Gemini rate limits" below — still relevant even on paid tier, since the per-*minute* `embed_content` limit isn't a free-tier-only thing.
